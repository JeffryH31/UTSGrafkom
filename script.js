function main() {
  ...
  //Punya Kevin
  var leftFootRotationXk = 0;
  var leftFootDirectionk = false;
  var rightFootRotationXk = 0;
  var rightFootDirectionk = false;
  var bodyTranslationZk = 0;
  var bodyDirectionk = false;
  var stopWalkingk = false;
  var bodyRotationYk = 0;
  ...
  var animate = function (time) {
      if (time > 0) {
          ...
          KEVIN_MATRIX = LIBS.get_I4();
          ANDREAS_MATRIX = LIBS.get_I4();
          ...
          LIBS.rotateY(KEVIN_MATRIX, THETA);
          LIBS.rotateX(KEVIN_MATRIX, ALPHA);
          LIBS.rotateX(KEVIN_MATRIX, timepichu);
          LIBS.rotateY(KEVIN_MATRIX, timepichu / 2);
          LIBS.rotateZ(KEVIN_MATRIX, -timepichu / 2);
          LIBS.translateX(KEVIN_MATRIX, -timepichu * 4);
          LIBS.translateY(KEVIN_MATRIX, timepichu);
          LIBS.translateZ(KEVIN_MATRIX, timepichu * 2);
          if (timepichu <= 2 && returnpichu == false) {

              LIBS.scalling(KEVIN_MATRIX, timepichu * 3, timepichu * 2, timepichu * 3);
              timepichu += 1 / 60;
              if (timepichu >= 1.99) {
                  returnpichu = true;
              }
          }
          else if (returnpichu == true) {

              if (timepichu >= 0.012) {
                  timepichu -= 1 / 60;

                  LIBS.scalling(KEVIN_MATRIX, 1 + (timepichu * 4 / 3), 1 + (timepichu * 4 / 3), 1 + (timepichu * 4 / 3));
              }
              if (timepichu <= 0.01) {
                  LIBS.scalling(KEVIN_MATRIX, 1, 1, 1);
              }
          }


          // KEVIN
          main.MODEL_MATRIX = KEVIN_MATRIX;
          body.MODEL_MATRIX = KEVIN_MATRIX;
          leftHand.MODEL_MATRIX = KEVIN_MATRIX;
          rightHand.MODEL_MATRIX = KEVIN_MATRIX;
          rightFoot.MODEL_MATRIX = KEVIN_MATRIX;
          leftFoot.MODEL_MATRIX = KEVIN_MATRIX;
          leftEye.MODEL_MATRIX = KEVIN_MATRIX;
          rightEye.MODEL_MATRIX = KEVIN_MATRIX;
          leftWhiteEye.MODEL_MATRIX = KEVIN_MATRIX;
          rightWhiteEye.MODEL_MATRIX = KEVIN_MATRIX;
          leftCheek.MODEL_MATRIX = KEVIN_MATRIX;
          rightCheek.MODEL_MATRIX = KEVIN_MATRIX;
          mouth.MODEL_MATRIX = KEVIN_MATRIX;
          leftEar.MODEL_MATRIX = KEVIN_MATRIX;
          rightEar.MODEL_MATRIX = KEVIN_MATRIX;
          tail.MODEL_MATRIX = KEVIN_MATRIX;
          tie1.MODEL_MATRIX = KEVIN_MATRIX;
          tie2.MODEL_MATRIX = KEVIN_MATRIX;
          nose.MODEL_MATRIX = KEVIN_MATRIX;
          ...
          LIBS.rotateY(ANDREAS_MATRIX, THETA);
          LIBS.rotateX(ANDREAS_MATRIX, ALPHA);
          timejump += 1 / 4;
          if (timejump <= 1) {
              LIBS.translateY(ANDREAS_MATRIX, jump);
              LIBS.scalling(ANDREAS_MATRIX, 0.9 + jump / 10, 0.9 + jump / 10, 0.9 + jump / 10);
              jump += 0.6;
          }
          else {
              jump = 0;
          ...
          headA.MODEL_MATRIX = ANDREAS_MATRIX;
          mouthA.MODEL_MATRIX = ANDREAS_MATRIX;
          bodyA.MODEL_MATRIX = ANDREAS_MATRIX;
          punggungA.MODEL_MATRIX = ANDREAS_MATRIX;
          leftHandA.MODEL_MATRIX = ANDREAS_MATRIX;
          rightHandA.MODEL_MATRIX = ANDREAS_MATRIX;
          rightFootA.MODEL_MATRIX = ANDREAS_MATRIX;
          leftFootA.MODEL_MATRIX = ANDREAS_MATRIX;
          leftEyeA.MODEL_MATRIX = ANDREAS_MATRIX;
          rightEyeA.MODEL_MATRIX = ANDREAS_MATRIX;
          leftWhiteEyeA.MODEL_MATRIX = ANDREAS_MATRIX;
          rightWhiteEyeA.MODEL_MATRIX = ANDREAS_MATRIX;
          tail1A.MODEL_MATRIX = ANDREAS_MATRIX;
          tail2A.MODEL_MATRIX = ANDREAS_MATRIX;
          noseA.MODEL_MATRIX = ANDREAS_MATRIX;
          nose1A.MODEL_MATRIX = ANDREAS_MATRIX;
